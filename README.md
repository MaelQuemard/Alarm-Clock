...
L’application fournie avec notre framework, est une application d’alarme. Elle nous permet de mettre en avant notre framework grâce aux différentes fonctionnalité que nous souhaitons lui ajouter. Dans notre cas, nous considérons que notre alarme doit dans un premier temps afficher l’heure actuelle, proposer des modifieurs pour la modifier sur demande, un manager d’alarmes, et enfin un afficheur.

Pour cela nous avons donc développé différents plugins qui représente ces fonctionnalités. Nous proposons donc une IHM simple mettant en avant  le chargement dynamique des dits plugins. Elle propose donc à l’utilisateur de charger les différentes fonctionnalitées une à une. Il est possible de proposer à l’utilisateur de choisir plusieurs plugins d’un coup, si la fonctionnalité se traduit par une liste de différent plugins (par exemple des modifieurs). Le résultat final est ensuite affiché.
...
Lors de l’ajout d’un nouveau plugin, il faut penser à ajouter son fonctionnement dans le plugin **“IApp”**, ainsi que les getter et setter munis du nom de l’extension suivi de l’annotation avec la valeur **“true”**.  Cette annotation permet de savoir si le getter ou le setter permet de récupérer ou de modifier un plugin lancé.
...
**“ExtensionLoader”** agit aussi comme Main. C’est dans ce main que sont lancées toutes les applications qui ont pour tag **“autorun”**. Une fois ces applications lancées, **“ExtensionLoader”** applique de façon cyclique la méthode **“IApp::run()”** a chaque application, tour à tour. Le run va donc jouer le rôle d’un main pour l‘application. 
A noter que les applications sont lancées dans le même Thread, un axe d’évolution consisterait à les lancer dans des Threads séparés. 
...
En voici la liste : 
 ...
 - Lancer nos IApp dans des Threads séparés serait plus pertinent que l’appel de façon cyclique à une méthode run(). De plus cela permettrait d'éviter un problème de famine à cause d’une application qui ne s'arrêterait pas dans son run. 

 - Rendre accessibles, par l’“ExtensionLoader”, les plugins lancés par d’autre plugin. Une solution serait d’y accéder de façon arborescente. C’est à dire, vérifier pour chaque plugin, si celui ci possède des attributs plugins et ainsi de suite. De cette façon on pourrait les afficher sous la forme d’un arbre dans le monitor. Il serait aussi nécessaire de modifier notre Handler pour pouvoir accéder à ces plugins.
 
 - Ajouter un système d'évent, comme certain groupes l’ont utiliser. Celui-ci aurait pour but d’offrir une implémentation différente de notre monitor. Permettant ainsi d’en faire un qui ne kill/remplace pas, mais liste juste les plugins lancé, et leur statut par exemple. Mais aussi de supprimer les dépendances entre les différents objets.
